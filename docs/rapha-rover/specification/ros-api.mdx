...
- `system/shutdown` ([std_msgs/msg/Empty])

  Performs system shutdown.

- `system/reboot` ([std_msgs/msg/Empty])
...
- `oak/imu/data` ([sensor_msgs/msg/Imu])
  ...
  Current gyroscope and accelerometer readings from the Oak IMU.

- `oak/left/camera_info` ([sensor_msgs/msg/CameraInfo])

  Calibration data for the left Oak camera.

- `oak/left/image_raw` ([sensor_msgs/msg/Image])

  Raw images from the left Oak camera.

- `oak/rgb/camera_info` ([sensor_msgs/msg/CameraInfo])

  Calibration data for the center (RGB) Oak camera.

- `oak/rgb/image_raw` ([sensor_msgs/msg/Image])

  Raw images from the center (RGB) Oak camera.

- `oak/right/camera_info` ([sensor_msgs/msg/CameraInfo])

  Calibration data for the right Oak camera.

- `oak/right/image_raw` ([sensor_msgs/msg/Image])

  Raw images from the right Oak camera.

- `oak/stereo/camera_info` ([sensor_msgs/msg/CameraInfo])

  Calibration data for **TODO**

- `oak/stereo/image_raw` ([sensor_msgs/msg/Image])

  Depth images calculated from the stereo pair (left and right Oak cameras).
...
- `rplidar/scan_filtered` ([sensor_msgs/msg/LaserScan]) **Not implemented yet**
  ...
  Laser scans from the onboard 2D LIDAR with points lying on the robots
...
- `drivetrain.max_wheel_angular_velocity` (type: `float`, default: `19.0`)
  ...
  Maximum allowable wheel angular velocity. [rad/s]

- `drivetrain.change_mode_acceleration` (type: `float`, default: `2.0`)

  Acceleration to use when changing steering mode. [m/s^2, rad/s^2]

- `drivetrain.change_mode_steering_angle_velocity` (type: `float`, default:
  `4.0`)

  Steering angle velocity to use when changing steering mode. [rad/s]
...
- `drivetrain.wheel_disable_timeout` (type: `float`, default: `10.0`)
  ...
  disable to motors. Set to 0 to disable. [s]
...
- `drivetrain.servo.position_loop_speed` (type: `float`, default: `5.0`)
  ...
  Position loop speed value passed to servos motors. [rad/s]
