...
To access microSD card from your Leo Rover, open the main electronics box by unscrewing the 4 socket-headed
screws and extract the card from the SD card slot on the Raspberry Pi.

Put the microSD card in the adapter and then connect it to your computer.

### Download the newest LeoOS image

You can find all releases of LeoOS [here](https://github.com/LeoRover/leo_os/releases). Choose the newest one
and download the image (.img.xz file) you want. We recommend the `full` version. The `lite` version comes without a desktop environment,
so it is an alternative if you don't need all the extra packages.
...
To download newest versions of system packages and ROS packages (including [leo_bringup](https://github.com/LeoRover/leo_bringup)
and [leo_ui](https://github.com/LeoRover/leo_ui.git)), connect to Leo Rover via SSH:
