---
title: Trossen Robotics WidowX 250 6DOF
sidebar_label: WidowX 250 6DOF
keywords:
  - robotic arm
  - payload
  - servo
  ...
  - example
description: >-
  Detailed tutorial on mounting the WidowX 250 6DOF arm, configuring ROS
  packages, and remotely operating the arm from your Leo Rover.
...
from being built. To build the packages for WidowX250 6DOF, remove the following
CATKIN_IGNORE files:
...
```bash
rostopic pub /wx250s/commands/joint_single interbotix_xs_msgs/JointSingleCommand "name: 'elbow' cmd: -0.5"
```
...
To launch the joy controller (using a ps4 gamepad), type:
...
```bash
roslaunch interbotix_xsarm_moveit moveit_rviz.launch rviz_frame:=wx250s/base_link
```
